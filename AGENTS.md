...
`src/main.rs` contains the Rust LCD service and is the main runtime path. `tools/` holds Python utilities for capture analysis and HID replay, especially `tools/lcdd_pcap.py` and `tools/lcdd_hid.py`, for reverse-engineering. `docs/` stores protocol and service notes. `src/assets/` contains packaged runtime assets, while `out/` is generated analysis output and should be treated as disposable. Top-level config examples such as `config.toml` live at the repository root.
...
```bash
cargo run -- --config ./config.toml   # run the Rust LCD service
