...
pub struct BladeOffscreenRenderer {
    ...
    readback_buffer: gpu::Buffer,
    size: gpu::Extent,
    surface_info: gpu::SurfaceInfo,
}

impl BladeOffscreenRenderer {
    pub fn new(context: &BladeContext, size: gpu::Extent, transparent: bool) -> anyhow::Result<Self> {
        let surface_info = gpu::SurfaceInfo {
            format: gpu::TextureFormat::Rgba8Unorm,
            alpha: if transparent {
                gpu::AlphaMode::PreMultiplied
            } else {
                gpu::AlphaMode::Ignored
            },
        };
        let command_encoder = context.gpu.create_command_encoder(gpu::CommandEncoderDesc {
            name: "offscreen",
        ...
        let atlas = Arc::new(BladeAtlas::new(&context.gpu));
        let atlas_sampler = context.gpu.create_sampler(gpu::SamplerDesc {
            name: "offscreen atlas sampler",
            mag_filter: gpu::FilterMode::Linear,
            min_filter: gpu::FilterMode::Linear,
            ..Default::default()
        });
        let (path_intermediate_texture, path_intermediate_texture_view) =
            create_path_intermediate_texture(&context.gpu, surface_info.format, size.width, size.height);
        let (path_intermediate_msaa_texture, path_intermediate_msaa_texture_view) =
            create_msaa_texture_if_needed(
                &context.gpu,
                surface_info.format,
            ...
            .unzip();
        let target_texture = context.gpu.create_texture(gpu::TextureDesc {
        ...
        let target_view = context.gpu.create_texture_view(
            target_texture,
            gpu::TextureViewDesc {
        ...
        let readback_buffer = context.gpu.create_buffer(gpu::BufferDesc {
    ...
    fn wait_for_gpu(&mut self) {
        if let Some(last_sp) = self.last_sync_point.take()
            && !self.gpu.wait_for(&last_sp, MAX_FRAME_TIME_MS)
        {
            log::error!("GPU hung during offscreen render");
            while !self.gpu.wait_for(&last_sp, MAX_FRAME_TIME_MS) {}
        }
    }
    ...
    pub fn draw(&mut self, scene: &Scene) {
        ...
        self.command_encoder.start();
        self.atlas.before_frame(&mut self.command_encoder);
        self.command_encoder.init_texture(self.target_texture);
        ...
        for batch in scene.batches() {
            match batch {
                ...
                PrimitiveBatch::Paths(paths) => {
                    ...
                    drop(pass);
                    self.draw_paths_to_intermediate(paths, self.size.width as f32, self.size.height as f32);
                    pass = self.command_encoder.render(
                        "offscreen-main",
                        gpu::RenderTargetSet {
                            colors: &[gpu::RenderTarget {
        ...
        let mut transfer = self.command_encoder.transfer("offscreen-readback");
        transfer.copy_texture_to_buffer(
            self.target_texture.into(),
            self.readback_buffer.into(),
            bytes_per_row,
            self.size,
        );
        drop(transfer);
    ...
    pub fn readback_image(&mut self) -> anyhow::Result<RgbaImage> {
        self.wait_for_gpu();
        let pixel_count = (self.size.width * self.size.height * 4) as usize;
        let data = unsafe { std::slice::from_raw_parts(self.readback_buffer.data(), pixel_count) };
        RgbaImage::from_raw(self.size.width, self.size.height, data.to_vec())
            .ok_or_else(|| anyhow::anyhow!("offscreen readback buffer had unexpected size"))
    }
}
